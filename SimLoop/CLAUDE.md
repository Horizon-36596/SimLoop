...
4. **Self-contained.** This module's tests, README, `LICENSE`, `PUBLISHING.md` and the `maven-publish` wiring at the bottom of `build.gradle` all live inside this directory, so the module carries everything it needs wherever it is checked out. Adding something the library needs to the repository root instead is how that stops being true.
